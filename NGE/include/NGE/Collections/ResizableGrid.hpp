...
namespace NGE::Collections {
	...
	public:
		...
		ResizableGrid(const NGE::Math::Vector2<std::size_t>& size, const TCell& defaultValue) : size(size), cells(size.x * size.y, defaultValue) {
			// ...
		}

		constexpr inline const NGE::Math::Vector2<std::size_t>& GetSize() const noexcept {
			return size;
		}
		...
		constexpr inline bool IsWithinBounds(std::size_t x, std::size_t y) const noexcept {
			return IsWithinBounds(NGE::Math::Vector2<std::size_t>(x, y));
		}
		
		constexpr inline bool IsWithinBounds(const NGE::Math::Vector2<std::size_t>& position) const noexcept {
			return (position.x < size.x) && (position.y < size.y);
		}

		constexpr inline std::size_t GetIndex(std::size_t x, std::size_t y) const {
			return GetIndex(NGE::Math::Vector2<std::size_t>(x, y));
		}
		
		constexpr inline std::size_t GetIndex(const NGE::Math::Vector2<std::size_t>& position) const {
		...
		{
			return GetCell(NGE::Math::Vector2<std::size_t>(x, y));
		}
		...
		constexpr const TCell& GetCell(const NGE::Math::Vector2<std::size_t>& position) const
		{
			return cells.at(GetIndex(position));
		}
		
		constexpr TCell& GetCell(const NGE::Math::Vector2<std::size_t>& position)
		{
			return cells.at(GetIndex(position));
		}

		constexpr inline bool TryGettingCell(std::size_t x, std::size_t y, TCell& result) const noexcept {
			return TryGettingCell(NGE::Math::Vector2<std::size_t>(x, y), result);
		}
		
		constexpr inline bool TryGettingCell(const NGE::Math::Vector2<std::size_t>& position, TCell& result) const noexcept {
		...
		{
			return SetCell(NGE::Math::Vector2<std::size_t>(x, y));
		}
		
		constexpr inline const bool SetCell(const NGE::Math::Vector2<std::size_t>& position, const TCell& cell) noexcept
		{
			bool ret(IsWithinBounds(position));
			if (ret) {
		...
		constexpr void Resize(const NGE::Math::Vector2<std::size_t>& size, EGridResizingRule gridResizingRule, const TCell* optionalDefaultCell = nullptr) {
			if (this->size != size) {
				...
				if (gridResizingRule == EGridResizingRule::Naive) {
					...
					NGE::Math::Vector2<long long> offset;
					...
					if (old_cell_count < new_cell_count) {
						...
						for (long long new_cell_index(static_cast<long long>(new_cell_count) - 1LL); new_cell_index >= 0LL; new_cell_index--) {
							NGE::Math::Vector2<long long> new_position(
								new_cell_count % static_cast<long long>(size.x),
								new_cell_count / static_cast<long long>(size.x)
							);
							NGE::Math::Vector2<long long> old_position = new_position + offset;
						...
						for (long long old_cell_index(static_cast<long long>(old_cell_count) - 1LL); old_cell_index >= 0LL; old_cell_index--) {
							NGE::Math::Vector2<long long> old_position(
								old_cell_count % static_cast<long long>(this->size.x),
								old_cell_count / static_cast<long long>(this->size.x)
							);
							NGE::Math::Vector2<long long> new_position = old_position - offset;
		...
		constexpr inline const TCell& operator [](const NGE::Math::Vector2<std::size_t>& position) const {
			return GetCell(position);
		}
		
		constexpr inline TCell& operator [](const NGE::Math::Vector2<std::size_t>& position) {
			return GetCell(position);
		}
	...
	private:
		...
		NGE::Math::Vector2<std::size_t> size;

		std::vector<TCell> cells;

		constexpr inline std::size_t GetIndexWithoutBoundsCheck(const NGE::Math::Vector2<std::size_t>& position) const noexcept {
