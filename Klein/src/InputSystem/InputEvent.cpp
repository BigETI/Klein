...
#include <Klein/InputSystem/EInputEventValueType.hpp>
...
InputEvent::InputEvent(const InputEvent& inputEvent, const Klein::Math::Vector2<float>& pressValue2D) :
	nameHash(inputEvent.nameHash),
	inputEventValueType(EInputEventValueType::AnalogPress2D),
	pressValue2D(pressValue2D) {
	// ...
...
InputEvent& InputEvent::operator =(const Klein::Math::Vector2<float>& pressValue2D) {
...
InputEvent& InputEvent::operator =(Klein::Math::Vector2<float>&& pressValue2D) {
	this->pressValue2D = pressValue2D;
	isPressing.reset();
	pressValue.reset();
	inputEventValueType = EInputEventValueType::AnalogPress2D;
