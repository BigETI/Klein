...
vector<InputEvent>& RaylibInputHandler::FetchInputEvents(vector<InputEvent>& result) {
	...
	if ((mouse_delta.x != 0.0f) || (mouse_delta.y != 0.0f)) {
		mouseDeltaInputEvent = InputEvent(mouseDeltaInputEvent, Klein::Math::Vector2<float>(mouse_delta.x, mouse_delta.y));
		result.push_back(mouseDeltaInputEvent);
	}
	::Vector2 temp_mouse_position(GetMousePosition());
	Klein::Math::Vector2<float> mouse_position(temp_mouse_position.x, temp_mouse_position.y);
	...
	if ((mouse_wheel_movement.x != 0.0f) || (mouse_wheel_movement.y != 0.0f)) {
		mouseWheelMovementInputEvent = InputEvent(mouseWheelMovementInputEvent, Klein::Math::Vector2<float>(mouse_wheel_movement.x, mouse_wheel_movement.y));
